...
from agents.diffusion import Diffusion, GoalDiffusion
...
class Diffusion_QL(object):
    def __init__(
        ...
        self.goal_diff = goal_diff.to(device)
    ...
    def load_model(self, dir, id=None):
        ...
        if id is not None:
            if id != "latest":
                assert isinstance(id, int), f"invalid load id: {id}"
                actor_load_path = f"{dir}/actor_{id}.pth"
                critic_load_path = f"{dir}/critic_{id}.pth"
            else:
                ckpt_files = glob(dir + "/*.pth")
                ckpt_epoch = sorted(
                    [
                ...
                latest_epoch = ckpt_epoch[-1]
                actor_load_path = f"{dir}/actor_{latest_epoch}.pth"
                critic_load_path = f"{dir}/critic_{latest_epoch}.pth"

        else:
            actor_load_path = f"{dir}/actor.pth"
            critic_load_path = f"{dir}/critic.pth"

        self.actor.load_state_dict(torch.load(actor_load_path))
        print(f"Load actor from {actor_load_path}")
        self.critic.load_state_dict(torch.load(critic_load_path))
        # self.critic_target.load_state_dict(torch.load(critic_load_path))
        print(f"Load critic from {critic_load_path}")
