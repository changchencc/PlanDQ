...
def skip_zero_dim(env=None):

    def _fn(dataset):

        observations = dataset['observations'].copy()
        # dims = np.where(np.abs(observations).sum(axis=0))[0]
        # zero_dims = np.where(np.abs(observations).sum(axis=0) == 0)[0]
        # print(f'zero dimensions are: {zero_dims}')
        # observations = observations[:, dims]

        # pdb.set_trace()
        maxs = observations.max(axis=0)
        mins = observations.min(axis=0)
        dims = np.where(maxs != mins)[0]
        zero_std_dims = np.where(maxs == mins)[0]
        print(f'const dimensions are: {zero_std_dims}')
        dataset['observations'] = observations[:, dims]

        return dataset

    return _fn

def skip_zero_dim_eval(dataset):
